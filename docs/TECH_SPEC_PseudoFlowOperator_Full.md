# Техническое задание: Kubernetes оператор **PseudoFlow Operator** — расширенная спецификация

> Основание: исходное ТЗ `TECH_SPEC_PseudoFlowOperator.md`, ваши два руководства `kube-apiserver-haproxy-metallb.md` и `nodelocaldns-setup.md`, плюс согласованные в переписке расширения DSL. Ничего не выдумано за пределами принятых допущений; там, где исходные манифесты отсутствуют (например, полный DaemonSet HAProxy), это явно отмечено как «нет в предоставленных материалах».
